...
//throws java.io.FileNotFoundException
  ...
  PrintWriter pwriter = new PrintWriter(output);
     ...
     // 1. line count
        ...
        pwriter.println("Result = " + res1.toString());
    ...
    // ****** your code starts here ******
     ...
     // 2. letter count, case-insensitive
        ...
        final Mapper map2 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {//97-122
              for (int i = 0;i<value.length();i++)
              {
                char c =Character.toLowerCase(value.charAt(i));
                if (c>=97&&c<=122)
                  mr.collect_map(""+(char)c,list("1"));
              }
        ...
        pwriter.println("Result = " + res2.toString());
     ...
     // 3. word count, case-insensitive
        ...
        pwriter.println("Result = " + res3.toString());
        ...
        pwriter.println("   " + ((Cons)first(lst)).toString());
      }
        ...
        final Mapper map4 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                int x = grepkey.length();
                for (int i=0;i+x<value.length();i++)
                {
                  String str = value.substring(i,i+x);
                  if (str.equals(grepkey))
                    mr.collect_map(str,list(""+key));
                }
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
              Cons lst = null;
              for (;value!=null;value=rest(value))
              {
                lst=append(lst,(Cons)first(value));
              }
              mr.collect_reduce(key, lst);
        ...
        pwriter.println("Result = " + res4.toString());
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
              ...
              mr.collect_map((String)first(lst),list(third(lst)));
              //System.out.printf("%s %s\n",first(lst),list(third(lst)));
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
              ...
              for (;value!=null;value=rest(value))
              {
                total += Integer.decode((String) first((Cons)
                                                         first(value)));
                count++;
              }
              average = (double)total/count;
              mr.collect_reduce(key, list(average,count));
